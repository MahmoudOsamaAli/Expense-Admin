...
public class RequestsAdapter extends RecyclerView.Adapter<RequestsAdapter.MyHolder> {
    ...
    public RequestsAdapter(Context mContext, ArrayList<PlaceModel> data) {
    ...
    @NonNull
    @Override
    public RequestsAdapter.MyHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.category_selected_item
                , parent, false);
        return new RequestsAdapter.MyHolder(view);
    }
...
//            setAnimation(holder.parent , position);
    ...
    public void changeData(ArrayList<PlaceModel> requests){
        data = new ArrayList<>();
